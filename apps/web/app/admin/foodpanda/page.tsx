...
export default function FoodpandaAnalyticsPage() {
  ...
  useEffect(() => {
    Promise.all([fetchAdminDashboard({ preset: "30d", segment: "all" }), fetchAdminDashboard({ preset: "30d", segment: "foodpanda" })])
      .then(([all, foodpanda]) => setDashboard({ ...foodpanda, breakdowns: all.breakdowns }))
      .catch((loadError) => setError(loadError instanceof Error ? loadError.message : "Failed to load Foodpanda analytics."))
      ...
      <AdminShell title="Foodpanda Analytics" description="See how much Foodpanda contributes to sales, and what the commission costs the business.">
        ...
        {loading || !dashboard ? <Card className="p-6 text-sm text-pocket-navy/60">Loading Foodpanda analytics...</Card> : (
          <>
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1.3fr)_minmax(0,0.7fr)]"><BISection title="Foodpanda trend" description="Gross platform revenue by day."><BILine entries={dashboard.series.map((entry) => ({ label: entry.label, value: entry.revenue }))} formatValue={formatCompactCurrency} /></BISection><BISection title="Revenue lost to commission" description="The estimate is intentionally visible so it can be replaced with settlement data later."><div className="rounded-2xl bg-pocket-cream p-5"><CircleDollarSign className="h-6 w-6 text-pocket-orange" /><p className="mt-4 break-words text-xl font-black leading-tight text-pocket-navy sm:text-2xl">{formatCompactCurrency(commission)}</p><p className="mt-1 text-sm text-pocket-navy/60">Estimated commission in the selected period.</p></div></BISection></div>
