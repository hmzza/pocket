...
export default function AdminPage() {
  ...
  return <div className="mx-auto max-w-[1400px] px-4 py-10 md:px-6"><AdminShell title="Overview" description="A focused business snapshot: performance, mix, product momentum, and the few actions that need attention now.">
    ...
    {loading || !dashboard ? <Card className="p-6 text-sm text-pocket-navy/60">Loading overview...</Card> : <>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><KPI label="Revenue" value={formatCompactCurrency(dashboard.summary.revenue)} helper={`${dashboard.summary.revenueDelta}% vs previous period`} accent /><KPI label="Revenue after Foodpanda cut" value={formatCompactCurrency(revenueAfterFoodpandaCut)} helper={`${formatCompactCurrency(foodpandaCommission)} Foodpanda commission`} /><KPI label="Orders" value={formatCompactNumber(dashboard.summary.orders)} helper={`${dashboard.summary.ordersDelta}% vs previous period`} /><KPI label="Average order value" value={formatCompactCurrency(dashboard.summary.averageOrderValue)} helper="Revenue per order" /><KPI label="Active customers" value={formatCompactNumber(dashboard.summary.activeCustomers)} helper="Customers who ordered" /></div>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><KPI label="Net profit" value={formatCompactCurrency(netProfit)} helper={`${netMargin.toFixed(1)}% net margin · after expenses`} /><KPI label="Food cost %" value="—" helper="Add recipe cost allocation" /><KPI label="Gross margin %" value="—" helper="Add recipe cost allocation" /><KPI label="Net margin %" value={`${netMargin.toFixed(1)}%`} helper="Revenue less operating expenses" /><KPI label="Foodpanda %" value={`${dashboard.summary.revenue ? (foodpandaRevenue / dashboard.summary.revenue * 100).toFixed(1) : "0.0"}%`} helper="Share of total revenue" /><KPI label="Direct sales %" value={`${dashboard.summary.revenue ? (directRevenue / dashboard.summary.revenue * 100).toFixed(1) : "0.0"}%`} helper="Inshop and other direct channels" /></div>
